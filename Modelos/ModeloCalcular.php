...
class ModeloCalcular
{
	...
	public static function mdlRegistrarimpuestoInafecto($datos)
	{
		...
		try {
			...
			// 							:total_aplicar,
			// 							:autovaluo)");
			// 	$stmt->bindParam(":periodo", $periodoUnPerido[$i]);
			...
			// else {
			// 	for ($i = 0; $i < count($periodoUnPerido); $i++) {
			// 		if ($i == 0) {
			// 			$saldo = $datos['gasto_emision'];
	...
	//REGISTRAR IMPUESTO CALCULAD EXONERADO PARCIALMENTE
	public static function mdlRegistrarimpuestoExoneradoParcialmente($datos)
	{
